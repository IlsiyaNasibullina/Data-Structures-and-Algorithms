...
/* Task 2.1.2
 The problem of line segment intersection has various applications in computer graphics, solid modeling, robotics, and
 geographic information systems. Complex intersection problems are broken down to simpler intersection
 subproblems and line segments intersection is one of them. One of the primary applications is in computer graphics,
 where it is essential for rendering three-dimensional objects on a two-dimensional screen.
 Ray shooting is also an important method for rendering scenes. The computationally most intensive part of
 ray shooting is determining the intersection of the ray with other objects. In solid modeling the usual task is to
 build up complex shapes by applying different operations (intersection, union, and
 difference) to simple primitive shapes. In order to perform these operations, the most basic step is determining the
 points where the boundaries of the two objects intersect. In robotics and motion planning this algorithm may be useful
 to know when two objects intersect for collision detection and collision avoidance. In geographic information systems
 it is often useful to overlay two subdivisions (e.g. a road network and county boundaries to determine where
 road maintenance responsibilities lie). Since these networks are formed from collections of line segments,
 this generates a problem of determining intersections of line segments. In the design of routing algorithms the algorithm
 is applied to find the shortest path between two points in a network of interconnected line segments.
 The efficient algorithms for line segment intersection has improved the visual representation of 3D objects
 in the development of more advanced computer graphics software. In summary, the problem of line segment intersection
 is crucial in various fields, and its efficient solution is essential for many real-world applications.
 */

/* Task 2.1.3
 The idea behind sweep line algorithm is to imagine that a line is swept or moved across the plane,
 stopping at some points. Geometric operations are restricted to geometric objects that either
 intersect or are in the immediate vicinity of the sweep line whenever it stops, and the complete solution is available
 once the line has passed over all objects.
 In the terms of our task we will use the algorithm in this way:
 firstly, we sort start and en points of our segments along the x coordinate from left to right, then we pass a line
 through all points from left to right and check intersections. We start from the leftmost point. If it is a start point
 of a segment, check if there is an intersection with the segment above and below it, add to active segments. If it is
 an end point, we remove the line segment from active segments and check if the segment above and below intersect with
 each other.
 */

/* Task 2.1.4
 I used quicksort sorting algorithm.
 - The time complexity is O(n log n) in best and average case and O(n^2) in worst case
 - It is in-place sorting algorithm as it sorts the element within the vector/array
 - My implementation of quicksort is unstable
 */
